...
### Mean/median number of loans per state + Ration of defaulter per state RESULTS - GEOGRAPHY
```{r}
...
# plot showing ratio of negative vs positive loan status ratio no/yes
loans_cleaned %>%
  group_by(purpose, defaulted) %>% 
  count(purpose) %>% 
  ggplot() +
  geom_col(aes(y = reorder(purpose, n),
               x = n,
               fill = defaulted),
           position = "fill") +
  lc_theme() +
...
### Linegraph - Fico_range_high
```{r}
loans_cleaned %>% 
  group_by(fico_range_high, defaulted) %>% 
  count(fico_range_high) %>% 
  ggplot() +
  aes(x = fico_range_high, y = n, colour = defaulted) +
  geom_line()
```
