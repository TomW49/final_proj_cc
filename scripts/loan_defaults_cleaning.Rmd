...
# discovered North Dakota was the NA' values
loans_cleaned <- loans %>%
...
# not including garde as have subgrade
...
onramp_data_trimmed  <- onramp_data_trimmed %>% 
   select(-c(emp_title, desc, title, loan_status, pymnt_plan, fico_range_low))
```
...
split2 <-  onramp_data_trimmed %>% 
  select(defaulted, sub_grade, emp_length, home_ownership, annual_inc, application_type)
